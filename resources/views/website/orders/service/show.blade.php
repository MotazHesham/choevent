                       ...
                       <div class="table-responsive">
                            <table class=" table table-bordered table-striped table-hover datatable datatable-Article">
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                      <td>تاريخ تجهيز الخدمة</td>
                                  <td>{{$order->setup_date}}</td>
                                  </tr>
                                  <tr>
                                      <td>عدد الأيام </td>
                                  <td>{{$order->days}}</td>
                                  </tr>
                                    @if($user->group=='provider' && $order->publish==1)
                                    <tr>
                                        <td>
                                            <a class="btn btn-xs btn-success" href="{{route('website.orders.confirm',['type'=>'service','id'=>$order->id])}}">
                                                الموافقة على الطلب
                                            </a>
                                            <a class="btn btn-xs btn-info" href="{{route('website.offers.create',['type'=>'service','order_id'=>$order->id,'event_id'=>$order->event->id])}}">
                                                إضافة عرض
                                            </a>
                                        </td>
                                        <td>
