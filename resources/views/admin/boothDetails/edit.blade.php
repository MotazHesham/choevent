        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.boothDetail.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.booth-details.update", [$boothDetail->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('order') ? 'has-error' : '' }}">
                            <label for="order">{{ trans('cruds.boothDetail.fields.order') }}</label>
                            <input class="form-control" type="number" name="order" id="order" value="{{ old('order', $boothDetail->order) }}" step="1">
                            @if($errors->has('order'))
                        ...
                        <div class="form-group {{ $errors->has('activity') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="activity" id="activity">
                                ...
                                @foreach(App\Models\BoothDetail::ACTIVITY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('activity', $boothDetail->activity) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('length') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="length" id="length" value="{{ old('length', $boothDetail->length) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('width') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="width" id="width" value="{{ old('width', $boothDetail->width) }}" step="0.01">
                            @if($errors->has('width'))
                        ...
                        <div class="form-group {{ $errors->has('booth') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="booth_id" id="booth_id">
                                @foreach($booths as $id => $booth)
                                    <option value="{{ $id }}" {{ (old('booth_id') ? old('booth_id') : $boothDetail->booth->id ?? '') == $id ? 'selected' : '' }}>{{ $booth }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('booth'))
