        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.configration.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.configrations.update", [$configration->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('item') ? 'has-error' : '' }}">
                            <label for="item">{{ trans('cruds.configration.fields.item') }}</label>
                            <input class="form-control" type="text" name="item" id="item" value="{{ old('item', $configration->item) }}" disabled>
                            @if($errors->has('item'))
                        ...
                        <div class="form-group {{ $errors->has('value') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="value" id="value" value="{{ old('value', $configration->value) }}">
                            @if($errors->has('value'))
